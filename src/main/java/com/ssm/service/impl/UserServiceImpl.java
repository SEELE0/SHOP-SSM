...
public class UserServiceImpl implements UserServiceInter {
	...
	public String login(String users, String password) throws Exception {
		...
		password= new Md5Hash(password).toString();//加密后查表
		UserP userP = userMapper.loginUsers(users, password);//查询用户是否存在
		if(userP!=null) {
			map.put("state", "1");
	...
	public String updatePwds(UserP userP,String oldPwd) throws Exception {
		...
		if(userP==null) {
			...
			oldPwd= new Md5Hash(oldPwd).toString();//加密
			if(!oldPwd.equals(p.getPassword())) {//判断旧密码是否正确0
				map.put("state", "0");
			}else {
				userP.setPassword(new Md5Hash(userP.getPassword()).toString());
...
//		});
	...
	public boolean addPlusers(UserP userP) throws Exception {
		...
		if(num>0) {
			bol = true;
			receMapper.addAddress(userP.getId());//创建成功后添加默认地址
		}else {
			bol = false;
		}
		return bol;
