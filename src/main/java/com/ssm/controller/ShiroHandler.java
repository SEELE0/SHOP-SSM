...
public class ShiroHandler {
	...
	public String updateAdminShiro(@PathVariable Integer shiroId,@PathVariable Integer userId) throws Exception {
		if(shiroId==null||userId==null) {
			return "<script>alert('非法请求！');location.href='/Shop-SSM/admin/AdminUserShiro.jsp'</script>";
		}else {
			boolean falg = shiroInter.AdminUpdateShi(shiroId, userId);
			if(falg) {
				return "<script>alert('修改成功');location.href='/Shop-SSM/admin/AdminUserShiro.jsp'</script>";
			}else {
				return "<script>alert('修改失败');location.href='/Shop-SSM/admin/AdminUserShiro.jsp'</script>";
			}
		}
	}
