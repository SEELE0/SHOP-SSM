...
public class OrdersHandler {
	...
	public String adminDaiDels(Integer id,Integer shiro_id) throws Exception {
		if(id==null||shiro_id==null) {
			return "<script>alert('非法请求');location.href='/Shop-SSM/admin/orders_dai.jsp';</script>";
		}else {
	...
	public String adminfahuoOrdes(Integer id) throws Exception{
		if(id==null) {
			return "<script>alert('非法请求');location.href='/Shop-SSM/admin/orders_weifa.jsp';</script>";
		}else {
			boolean falg = ordersServiceInter.adminFaOrdes(id);
			if(falg) {
				return "<script>alert('发货成功');location.href='/Shop-SSM/admin/orders_weifa.jsp';</script>";
			}else {
				return "<script>alert('发货失败');location.href='/Shop-SSM/admin/orders_weifa.jsp';</script>";
			}
		}
	}
