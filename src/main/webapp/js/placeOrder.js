$(function () {
    ...
    function ordersList_() {//将订单拼接，然后放入数组中，最后用来一起提交
        for (var i = 0; i < $('.uls').length; i++) {
            var $aa = $('.uls').eq(i);
            var dan = {
                "order_userid": eval("(" + $.cookie('shop-users') + ")")['id'],
                "address": $('#dsd').html(),
                ...
                "goods_id": goodsCartList[i]["goods_id"]
            }
            pays.push(dan);
        }
    ...
    var goodsCartIds = localStorage.getItem("GoodsCartIds");
    console.log("goodsCartIds:" + goodsCartIds);
    $.ajax({
        url: "/Shop-SSM/getByIdGoodsCartId.do",
        type: "post",
        data: {
            "cartIds": goodsCartIds
            // eval("(" + $.cookie('shop-users') + ")")['id']
        },
        dataType: "json",
        success: function (m) {
            goodsCartList = m;
